...
def draw_neural_net(ax, left, right, bottom, top, layer_sizes, coefs_, intercepts_,n_iter_, loss_, np, plt):
    ...
    for m in xrange(layer_sizes[0]):
        # plt.arrow(0, layer_top_0 - m*v_spacing, 0.20, 0,  lw =1, head_width=0.01, head_length=0.02)
        ax.annotate("",
            xy=(0, layer_top_0 - m*v_spacing), xycoords='data',
            xytext=(0, layer_top_0 - m*v_spacing), textcoords='data',
    ...
    for m in xrange(layer_size_a):
            ...
            for o in xrange(layer_size_b):
                ...
                line = plt.Line2D([x_bias, (n + 1)*h_spacing + left],
                              [biasY, layer_top_b - o*v_spacing], c='k')
                ax.add_artist(line)
                xo = ((n + 1)*h_spacing + left)
                yo = (layer_top_b - o*v_spacing)
